...
from verl.trainer.exp_ppo import core_algos
from verl.trainer.exp_ppo.core_algos import AdvantageEstimator, agg_loss
from verl.trainer.exp_ppo.metric_utils import (
    compute_data_metrics,
    compute_throughout_metrics,
    compute_timing_metrics,
    process_validation_metrics,
)
from verl.trainer.exp_ppo.reward import compute_reward, compute_reward_async
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            # AdvantageEstimator.GPG, #zr version conflicts
        ]:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        if self.config.get("ttrl", {}).get("enable", False):
                            from verl.trainer.exp_ppo.ttrl_utils import select_top_k_per_prompt, apply_ttrl_gt, select_top_k_per_prompt_result
                            ...
                            batch = select_top_k_per_prompt_result(
                                batch,
                                self.config.ttrl.n_votes_per_prompt,
                                self.config.ttrl.n_samples_per_prompt,
                            )
                    ...
                    if not repeat_sampling_sglang_grpo:
                        ...
                        if self.config.get("ttrl", {}).get("enable", False):
                            rollout_obj = [
                                obj
                                for data_item in batch
                                for obj in data_item.non_tensor_batch["extra_info"]["solved_objective"]
                            ]
                            rollout_sol = [
                                sol
                                for data_item in batch
                                for sol in data_item.non_tensor_batch["extra_info"]["solution"]
                            ]
                            rollout_code = [
                                rep
                                for data_item in batch
                                for rep in data_item.non_tensor_batch["extra_info"]["code_exec_res"]
                            ]
                    ...
                    if self.config.get("ttrl", {}).get("enable", False):
                        ##zr ttrl second: 
                        from verl.trainer.exp_ppo.ttrl_utils import apply_original_gt, compute_ttrl_metrics
